...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 6) {
      // Add backlog_at column (old learning_at represented "when first added")
      final cols = await db.rawQuery('PRAGMA table_info(pieces)');
      final hasBacklogAt = cols.any((c) => c['name'] == 'backlog_at');
      if (!hasBacklogAt) {
        await db.execute(
          'ALTER TABLE pieces ADD COLUMN backlog_at TEXT',
        );
      }

      // Migrate timestamps: old learning_at → backlog_at for all pieces
      await db.execute(
        'UPDATE pieces SET backlog_at = learning_at WHERE backlog_at IS NULL',
      );

      // Old "learning" pieces → backlog (clear learning_at, they haven't started active learning)
      await db.execute(
        "UPDATE pieces SET learning_at = NULL WHERE status = 'learning'",
      );

      // Migrate status values
      await db.execute(
        "UPDATE pieces SET status = 'backlog' WHERE status = 'learning'",
      );
      await db.execute(
  ...
  Future<int> deletePiece(int id) async {
    final db = await database;
    await db.delete('practice_sessions', where: 'piece_id = ?', whereArgs: [id]);
  ...
  Future<void> seedTestData() async {
    ...
    final pieces = <Map<String, dynamic>>[
      ...
      _seed(now, 'Invention No. 1', 'Bach', kStageLearning, 44, ml: 44, ct: 96, tt: 120, daysAgo: 30),
      _seed(now, 'Waltz Op. 64 No. 2', 'Chopin', kStageLearning, 80, ml: 80, ct: 132, tt: 176, daysAgo: 35),
      _seed(now, 'Sonatina in G', 'Beethoven', kStageLearning, 68, ml: 68, ct: 100, tt: 140, daysAgo: 28),
      _seed(now, 'Liebestraum No. 3', 'Liszt', kStageLearning, 90, ml: 90, ct: 60, tt: 84, daysAgo: 40),
      _seed(now, 'Spring (Four Seasons)', 'Vivaldi', kStageLearning, 120, ml: 120, ct: 88, tt: 132, daysAgo: 32),
      _seed(now, 'Prelude Op. 28 No. 4', 'Chopin', kStageLearning, 48, ml: 48, ct: 48, tt: 60, daysAgo: 38),
      _seed(now, 'Waldstein Sonata', 'Beethoven', kStageLearning, 302, ml: 302, ct: 120, tt: 168, daysAgo: 45),
      _seed(now, 'Etude Op. 10 No. 1', 'Chopin', kStageLearning, 79, ml: 79, ct: 116, tt: 176, daysAgo: 42),
      _seed(now, 'Gnossienne No. 1', 'Satie', kStageLearning, 88, ml: 88, ct: 50, tt: 60, daysAgo: 50),
      _seed(now, 'Minuet in G', 'Bach', kStageLearning, 32, ml: 32, ct: 100, tt: 120, daysAgo: 48),
      _seed(now, 'Pathetique Sonata', 'Beethoven', kStageLearning, 352, ml: 352, ct: 138, tt: 152, daysAgo: 60),
      _seed(now, 'La Campanella', 'Liszt', kStageLearning, 188, ml: 188, ct: 168, tt: 184, daysAgo: 55),

      // ── Repertoire (10 pieces) ────────────────────────────────────────────
      ...
      _seed(now, 'Nocturne Op. 27 No. 2', 'Chopin', kStageRepertoire, 110, ml: 110, ct: 52, tt: 58, daysAgo: 90),
      _seed(now, 'Goldberg Variations', 'Bach', kStageRepertoire, 320, ml: 320, ct: 80, tt: 88, daysAgo: 110),
      _seed(now, 'Rhapsody in Blue', 'Gershwin', kStageRepertoire, 400, ml: 400, ct: 112, tt: 126, daysAgo: 100),
      _seed(now, 'Consolation No. 3', 'Liszt', kStageRepertoire, 68, ml: 68, ct: 50, tt: 56, daysAgo: 85),
      _seed(now, 'Etude Op. 25 No. 11', 'Chopin', kStageRepertoire, 56, ml: 56, ct: 88, tt: 160, daysAgo: 130),
      _seed(now, 'Sonatine', 'Ravel', kStageRepertoire, 112, ml: 112, ct: 88, tt: 116, daysAgo: 75),
      _seed(now, 'Comptine d\'un autre été', 'Tiersen', kStageRepertoire, 56, ml: 56, ct: 84, tt: 84, daysAgo: 70),
    ];
    ...
    // Seed practice sessions for the first 20 pieces
    for (var i = 0; i < 20 && i < insertedIds.length; i++) {
      final pieceId = insertedIds[i];
      final sessionCount = (i % 4) + 1;
