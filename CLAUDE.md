...
- **Keyboard blocking bottom-sheet buttons**: After `enterText` inside a modal bottom sheet, the soft keyboard remains up and its hit-test layer blocks taps on buttons near the bottom of the sheet (e.g. "Save Session"). Use the `_dismissKeyboard(tester)` helper which calls unfocus + pumpAndSettle + pump(500ms) + pumpAndSettle. The extra pump is needed on API 37 where the OS keyboard dismiss animation outlasts Flutter's settle cycle.
